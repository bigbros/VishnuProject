...
C3DMaterial::setup(C3DDefaultShader * shader)
{
	if (m_switch & TEXTURE) {
		...
		glUniform1i(shader->m_u_texmap, GL_TRUE);
	}
	else {
		glUniform1i(shader->m_u_texmap, GL_FALSE);
	}
	if (m_switch & NORMAL) {
		m_normal->set(GL_TEXTURE1);
		glUniform1i(shader->m_u_normmap, 1);
		glUniform1i(shader->m_u_normalmap, GL_TRUE);
	}
	else {
		glUniform1i(shader->m_u_normalmap, GL_FALSE);
	}
